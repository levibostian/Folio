...
/// LICENSE: MIT: https://github.com/lukaskubanek/OrderedDictionary/blob/master/LICENSE.md
...
public struct OrderedDictionary<Key: Hashable, Value>: BidirectionalCollection {
    ...
    public init(unsorted: [Key: Value],
                areInIncreasingOrder: (Element, Element) throws -> Bool) rethrows {
    ...
    public init<S: Sequence>(values: S,
                             uniquelyKeyedBy extractKey: (Value) throws -> Key) rethrows where S.Element == Value {
    ...
    public init<S: Sequence>(values: S,
                             uniquelyKeyedBy keyPath: KeyPath<Value, Key>) where S.Element == Value {
    ...
    public var keys: OrderedDictionaryKeys<Key, Value> {
    ...
    // Removed because I don't like how we can do this: `orderedDictionary[key] = value` as that does not specify *where* we want to insert the item, which causes bugs.
...
//    }
    ...
    public mutating func insertLast(_ newElement: Element) {
        let index = count
    ...
    public mutating func update(_ newElement: Element, at index: Index) -> Element? {
        ...
        precondition(_canUpdate(newElement, at: index, keyPresentAtIndex: &keyPresentAtIndex),
    ...
    private func _canUpdate(_ newElement: Element,
                            at index: Index,
                            keyPresentAtIndex: inout Bool) -> Bool {
    ...
    public func mapValues<T>(_ transform: (Value) throws -> T) rethrows -> OrderedDictionary<Key, T> {
        ...
        for (key, value) in self {
            result.updateValue(try transform(value), forKey: key)
        }
    ...
    public func compactMapValues<T>(_ transform: (Value) throws -> T?) rethrows -> OrderedDictionary<Key, T> {
        ...
        for (key, value) in self {
            if let transformedValue = try transform(value) {
                result.updateValue(transformedValue, forKey: key)
            }
        }
...
extension Dictionary {
    ...
    public func sorted(by areInIncreasingOrder: (Element, Element) throws -> Bool) rethrows -> OrderedDictionary<Key, Value> {
        return try OrderedDictionary(unsorted: self,
